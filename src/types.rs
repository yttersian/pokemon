...
// Source: [Generations VI](https://bulbapedia.bulbagarden.net/wiki/Type/Type_chart)
#[rustfmt::skip]
const TYPE_TABLE: [[u8; 17]; 17] = [
//  No Fi Fl Po Gd Ro Bg Gh St Fi Wa Gs El Ps Ic Dr Da
...
mod tests {
    ...
    #[test]
    fn test_type_table() {
        assert_eq!(get_effectiveness(Fire, Grass), SuperEffective);
        assert_eq!(get_effectiveness(Water, Grass), NotVeryEffective);
        assert_eq!(get_effectiveness(Fighting, Ghost), NoEffect);
        assert_eq!(get_effectiveness(Normal, Normal), Neutral);
    }

    #[test]
    fn test_get_effectiveness_by_match() {
        assert_eq!(_get_effectiveness(Fire, Grass), SuperEffective);
        assert_eq!(_get_effectiveness(Water, Grass), NotVeryEffective);
        assert_eq!(_get_effectiveness(Fighting, Ghost), NoEffect);
        assert_eq!(_get_effectiveness(Normal, Normal), Neutral);
    }
}
